...
class MinkResNet_Two(ResNetBase):
    BLOCK = BasicBlock
    DILATIONS = (1, 1, 1, 1, 1, 1, 1, 1)
    LAYERS = (2, 2, 2, 2, 2, 2, 2, 2)
    PLANES = (32, 64, 128, 256, 256, 128, 96, 96)
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        self.final = nn.Sequential(
            self.get_mlp_block(self.embedding_channel * 2, out_channels),
            # ME.MinkowskiDropout(),
            # self.get_mlp_block(1024, 1024),
            # ME.MinkowskiLinear(self.embedding_channel * 2, out_channels, bias=True),
        )
    ...
    def forward(self, xyz, features, device="cuda", quantization_size=0.05):
        ...
        return self.final(ME.cat(x1, x2))
        # return ME.cat(x1, x2)

class MinkResNet34_Two(MinkResNet_Two):
    LAYERS = (3, 4, 6, 3)
